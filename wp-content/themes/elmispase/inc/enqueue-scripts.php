<?php
/**
 * Elmispase enqueue CSS and JS files.
 *
 * @package    ThemeGrill
...
function elmispase_scripts_styles_method() {
	...
	if ( false === apply_filters( 'elmispase_enqueue_theme_assets', true ) ) {
		return;
	}
	...
	wp_style_add_data( 'elmispase_style', 'rtl', 'replace' );

	if ( get_theme_mod( 'elmispase_color_skin', 'light' ) == 'dark' ) {
		wp_enqueue_style( 'elmispase_dark_style', ELMISPASE_CSS_URL . '/dark.css' );
	}

	// Add Genericons, used in the main stylesheet.
	wp_enqueue_style( 'elmispase-genericons', get_template_directory_uri() . '/genericons/genericons.css', array(), '3.3.1' );

	// Enqueue font-awesome style.
	wp_enqueue_style( 'elmispase-font-awesome', get_template_directory_uri() . '/font-awesome/css/font-awesome.min.css', array(), '4.7.1' );

	/**
	 * Inline CSS for this theme.
	 */
	add_filter( 'elmispase_dynamic_theme_css', array( 'Elmispase_Dynamic_CSS', 'render_output' ) );

	// Enqueue required Google font for the theme.
	Elmispase_Generate_Fonts::render_fonts();

	// Generate dynamic CSS to add inline styles for the theme.
	$theme_dynamic_css = apply_filters( 'elmispase_dynamic_theme_css', '' );
	if ( get_theme_mod( 'elmispase_color_skin', 'light' ) == 'dark' ) {
		wp_add_inline_style( 'elmispase_dark_style', $theme_dynamic_css );
	} else {
		wp_add_inline_style( 'elmispase_style', $theme_dynamic_css );
	}
	 ...
	 */
	wp_register_script( 'jquery_cycle', ELMISPASE_JS_URL . '/jquery.cycle2.min.js', array( 'jquery' ), '2.1.6', true );
	wp_register_script( 'jquery-swipe', ELMISPASE_JS_URL . '/jquery.cycle2.swipe.min.js', array( 'jquery' ), false, true );

	/**
	 * Enqueue Slider setup js file.
	 */
	if ( is_home() || is_front_page() && get_theme_mod( 'elmispase_activate_slider', '0' ) == '1' ) {
	...
	wp_enqueue_script( 'elmispase-skip-link-focus-fix', ELMISPASE_JS_URL . '/skip-link-focus-fix.js', array(), false, true );
 ...
 */
function elmispase_get_fonts() {

	/*
	 * Global
	 */
	$elmispase_content_font_typography_default      = array(
		'font-family' => 'Lato',
	);
	$elmispase_titles_font_typography_default       = array(
		'font-family' => 'Lato',
	);
	$elmispase_site_title_font_typography_default   = array(
		'font-family' => 'Lato',
	);
	$elmispase_site_tagline_font_typography_default = array(
		'font-family' => 'Lato',
	);
	$elmispase_primary_menu_font_typography_default = array(
		'font-family' => 'Lato',
	);
	$elmispase_content_font_typography              = get_theme_mod( 'elmispase_content_font_typography', $elmispase_content_font_typography_default );
	$elmispase_titles_font_typography               = get_theme_mod( 'elmispase_titles_font_typography', $elmispase_titles_font_typography_default );

	Elmispase_Generate_Fonts::add_font( $elmispase_content_font_typography['font-family'] );
	Elmispase_Generate_Fonts::add_font( $elmispase_titles_font_typography['font-family'] );
}

add_action( 'elmispase_get_fonts', 'elmispase_get_fonts' );


if ( ! function_exists( 'elmispase_parse_css' ) ) :
	 ...
	 */
	function elmispase_parse_css( $default_value, $output_value, $css_output = array(), $min_media = '', $max_media = '' ) {
...
if ( ! function_exists( 'elmispase_parse_background_css' ) ) :
	 ...
	 */
	function elmispase_parse_background_css( $default_value, $output_value, $selector ) {
...
if ( ! function_exists( 'elmispase_parse_typography_css' ) ) :
	 ...
	 */
	function elmispase_parse_typography_css( $default_value, $output_value, $selector, $devices = array() ) {
