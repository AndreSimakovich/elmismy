 ...
 */
function elmispase_demo_import_migration_notice() {
	$demo_imported  = get_option( 'themegrill_demo_importer_activated_id' );
	$notice_dismiss = get_option( 'elmispase_demo_import_migration_notice_dismiss' );

	if ( ! $notice_dismiss ) :

		if ( $demo_imported && ( strpos( $demo_imported, 'elmispase' ) !== false ) ) :
			?>
			<div class="notice notice-success elmispase-notice demo-import-migrate-notice" style="position:relative;">
				<p>
					<?php
					esc_html_e(
				...
				<p>
					...
					>
						<span><?php esc_html_e( 'Fix Imported Demo', 'elmispase' ); ?></span>
					</a>

					<a href="<?php echo esc_url( 'https://themegrill.com/contact/' ); ?>" class="btn button-secondary" target="_blank">
						<span><?php esc_html_e( 'Any confusion?', 'elmispase' ); ?></span>
					</a>
				</p>
...
add_action( 'admin_notices', 'elmispase_demo_import_migration_notice' );


/**
 * Option to dismiss the notice.
 */
function elmispase_demo_import_migration_notice_dismiss() {
	if ( isset( $_GET['demo-import-migration-notice-dismiss'] ) && isset( $_GET['_demo_import_migration_notice_dismiss_nonce'] ) ) {
		if ( ! wp_verify_nonce( $_GET['_demo_import_migration_notice_dismiss_nonce'], 'demo_import_migration_notice_dismiss' ) ) {
			wp_die( __( 'Action failed. Please refresh the page and retry.', 'elmispase' ) );
		}
...
add_action( 'admin_init', 'elmispase_demo_import_migration_notice_dismiss' );
 ...
 * @return bool
 */
function elmispase_demo_import_migration() {
