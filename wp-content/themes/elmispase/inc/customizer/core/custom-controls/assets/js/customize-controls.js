 ...
 * @package Elmispase
 */
wp.customize.controlConstructor[ 'elmispase-buttonset' ] = wp.customize.Control.extend( {
...
(
	function ( $ ) {
		...
		wp.customize.controlConstructor['elmispase-background'] = wp.customize.Control.extend( {
			...
			ready : function () {
				...
				control.initElmispaseBackgroundControl();

			},

			initElmispaseBackgroundControl : function () {

				var control     = this,
				    value       = control.setting._value,
				    colorpicker = control.container.find( '.elmispase-color-picker-alpha' );
				...
				control.container.on( 'click', '.background-image-upload-remove-button', function ( e ) {
					...
					if ( preview.length ) {
						preview.removeClass().addClass( 'placeholder' ).html( ElmispaseCustomizerControlBackground.placeholder );
					}
...
(
	function ( $ ) {
		...
		wp.customize.controlConstructor[ 'elmispase-color' ] = wp.customize.Control.extend( {
			...
			ready : function () {
				...
				var control = this,
					...
					picker = this.container.find( '.elmispase-color-picker-alpha' ),
 ...
 * @package Elmispase
 */
wp.customize.controlConstructor['elmispase-dimensions'] = wp.customize.Control.extend( {
 ...
 * @package Elmispase
 */
wp.customize.controlConstructor[ 'elmispase-dropdown-categories' ] = wp.customize.Control.extend( {
 ...
 * @package Elmispase
 */
wp.customize.controlConstructor[ 'elmispase-editor' ] = wp.customize.Control.extend( {
...
(
	function ( $ ) {
		...
		wp.customize.controlConstructor['elmispase-fontawesome'] = wp.customize.Control.extend(
			{
				ready: function () {
					'use strict';
					...
					control.initElmispaseFontawesomeControl();
				},

				initElmispaseFontawesomeControl: function() {
					var control       = this,
						selector      = control.selector,
						elSelector    = $( selector ).find( 'select' ),
						faData        = [],
						value         = control.setting._value,
						data          = window['ElmispaseCustomizerControlFontawesome' + this.id],
						faDataCounter = 0,
						faSelect;
...
(
	function ( $ ) {
		...
		wp.customize.controlConstructor['elmispase-group'] = wp.customize.Control.extend( {
			...
			ready : function () {
				...
				this.container.on( 'elmispase_settings_changed', control.onOptionChange );
			...
			registerToggleEvents : function () {
				...
				$( '.wp-full-overlay-sidebar-content' ).click(
					function ( e ) {
						if ( ! $( e.target ).closest( '.elmispase-field-settings-modal' ).length ) {
							$( '.elmispase-group-toggle-icon.open' ).trigger( 'click' );
						}
					}
				);
				...
				control.container.on(
					'click',
					'.elmispase-group-wrap .elmispase-group-toggle-icon',
					function ( e ) {
						...
						var $this          = $( this ),
						    parent_wrap    = $this.closest( '.customize-control-elmispase-group' ),
						    is_loaded      = parent_wrap.find( '.elmispase-field-settings-modal' ).data( 'loaded' ),
						    parent_section = parent_wrap.parents( '.control-section' );

						if ( $this.hasClass( 'open' ) ) {
							parent_wrap.find( '.elmispase-field-settings-modal' ).hide();
						} else {

							/* Close popup when another popup is clicked */
							var get_open_popup = parent_section.find( '.elmispase-group-toggle-icon.open' );
							if ( get_open_popup.length > 0 ) {
								get_open_popup.trigger( 'click' );
							}

							if ( is_loaded ) {
								parent_wrap.find( '.elmispase-field-settings-modal' ).show();
							} else {

								var fields     = control.params.elmispase_fields,
								    modal_wrap = $( ElmispaseCustomizerControlGroup.group_modal_tmpl ),
								    device     = $( '#customize-footer-actions .active' ).attr( 'data-device' );

								parent_wrap.find( '.elmispase-field-settings-wrap' ).append( modal_wrap );
								parent_wrap.find( '.elmispase-fields-wrap' ).attr( 'data-control', control.params.name );
								control.elmispase_render_field( parent_wrap, fields, control );
								parent_wrap.find( '.elmispase-field-settings-modal' ).show();
				...
				control.container.on(
					'click',
					'.elmispase-group-wrap > .customizer-text',
					function ( e ) {

						e.preventDefault();
						e.stopPropagation();

						$( this ).find( '.elmispase-group-toggle-icon' ).trigger( 'click' );
			...
			elmispase_render_field : function ( wrap, fields, control_element ) {

				var control             = this;
				var elmispase_field_wrap = wrap.find( '.elmispase-fields-wrap' );
				var fields_html         = '';
				var control_types       = [];
				...
				if ( 'undefined' != typeof fields.tabs ) {
					...
					var counter = 0;
					fields_html += '<div id="' + control_element.params.name + '-tabs" class="elmispase-group-tabs">';

					fields_html += '<ul class="elmispase-group-list">';
					_.each(
						fields.tabs,
						function ( value, key ) {
							var li_class = '';
					...
					fields_html += '<div class="elmispase-tab-content" >';
					_.each(
						fields.tabs,
					...
					elmispase_field_wrap.html( fields_html );
				...
				_.each(
					...
					function ( control_type, index ) {
						...
						switch ( control_type.key ) {
							...
							case 'elmispase-color':
								control.initColorControl( elmispase_field_wrap, control_element, control_type.name );
								break;

							case 'elmispase-background':
								control.initBackgroundControl( control_element, control_type, control_type.name );
								break;

							case 'elmispase-typography':
				...
				wrap.find( '.elmispase-field-settings-modal' ).data( 'loaded', true );
			...
			generateFieldHtml : function ( fields_data, field_values ) {
				...
				_.each(
					...
					function ( attr, index ) {
						...
						if ( (
							     'elmispase-typography' === attr.control
						     ) && controlsType.includes( attr.control ) ) {
							attr.languages = ElmispaseCustomizerControlTypographySubsets;
						}
			...
			initColorControl : function ( wrap, control_elem, name ) {
				...
				var control     = this;
				var colorpicker = wrap.find( '.customize-control-elmispase-color .elmispase-color-picker-alpha' );
				...
				colorpicker.wpColorPicker(
					{
						change : function ( event, ui ) {
							...
							if ( 'undefined' != typeof event.originalEvent || 'undefined' != typeof ui.color._alpha ) {
								...
								var picker  = $( '#' + name + '.customize-control-elmispase-color .elmispase-color-picker-alpha' );
			...
			getTypographyFonts : function () {
				...
				if ( ! _.isUndefined( ElmispaseCustomizerControlTypography ) ) {
					return ElmispaseCustomizerControlTypography;
				}
 ...
 * @package Elmispase
 */
wp.customize.controlConstructor[ 'elmispase-radio-image' ] = wp.customize.Control.extend( {
 ...
 * @package Elmispase
 */
wp.customize.controlConstructor['elmispase-slider'] = wp.customize.Control.extend( {
	...
	ready : function () {
		...
		this.container.find( 'input[type=range]' ).on( 'input change', function () {
			var value        = jQuery( this ).val(),
				input_number = jQuery( this ).closest( '.slider-wrapper' ).find( '.elmispase-range-value .value' );
		...
		this.container.find( '.elmispase-slider-reset' ).click( function () {
			var wrapper       = jQuery( this ).closest( '.slider-wrapper' ),
			    input_range   = wrapper.find( 'input[type=range]' ),
			    input_number  = wrapper.find( '.elmispase-range-value .value' ),
 ...
 * @package Elmispase
 */
wp.customize.controlConstructor['elmispase-sortable'] = wp.customize.Control.extend( {
 ...
 * @package Elmispase
 */
wp.customize.controlConstructor['elmispase-toggle'] = wp.customize.Control.extend( {
 ...
 * @package Elmispase
 */
wp.customize.controlConstructor['elmispase-typography'] = wp.customize.Control.extend( {
