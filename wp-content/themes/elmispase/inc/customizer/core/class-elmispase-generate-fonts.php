 ...
 * Helper class to enqueue fonts.
 *
 * Class Elmispase_Generate_Fonts
 */
class Elmispase_Generate_Fonts {
	...
	public static function get_fonts() {
		...
		do_action( 'elmispase_get_fonts' );

		return apply_filters( 'elmispase_add_fonts', self::$fonts );
	...
	public static function render_fonts() {
		...
		$font_list = apply_filters( 'elmispase_render_fonts', self::get_fonts() );

		$google_fonts = array();
		$font_subset  = array();

		$system_fonts = Elmispase_Fonts::get_system_fonts();
		...
		foreach ( $font_list as $name => $font ) {
			if ( ! empty( $name ) && ! isset( $system_fonts[ $name ] ) ) {
				...
				// Add Subset.
				$subset = apply_filters( 'elmispase_font_subset', '', $name );
				if ( ! empty( $subset ) ) {
		...
		wp_enqueue_style( 'elmispase_googlefonts', $google_font_url, array(), ELMISPASE_THEME_VERSION, 'all' );
	...
	public static function google_fonts_url( $fonts, $subsets = array() ) {
		...
		$fonts = apply_filters( 'elmispase_google_fonts_selected', $fonts );
