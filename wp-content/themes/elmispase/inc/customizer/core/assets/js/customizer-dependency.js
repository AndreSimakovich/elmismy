...
(
	function ( $ ) {
		 ...
		 */
		var Elmispase_Customizer_FrameWork = {
			...
			checkControlVisibility : function ( control, id ) {
				...
				if ( ! _.isUndefined( control ) ) {
					...
					if ( 'undefined' != typeof ElmispaseCustomizerControlsToggle[id] ) {
						var check            = false,
						    dependency_param = ElmispaseCustomizerControlsToggle[id],
						...
						if ( 'undefined' !== typeof conditions ) {
							...
							if ( ! check ) {
								control.container.addClass( 'elmispase-hide' );
							} else {
								control.container.removeClass( 'elmispase-hide' );
							}
						}
					}
			...
			checkDependency : function ( conditions, values, compare_operator ) {
				...
				if ( _.isString( testValue ) ) {
					...
					if ( ! _.isUndefined( ElmispaseCustomizerControlsToggle[testValue] ) ) {
						var conditions = ! _.isUndefined( ElmispaseCustomizerControlsToggle[testValue]['conditions'] ) ? ElmispaseCustomizerControlsToggle[testValue]['conditions'] : ElmispaseCustomizerControlsToggle[testValue];
						var operator   = ! _.isUndefined( ElmispaseCustomizerControlsToggle[testValue]['operator'] ) ? ElmispaseCustomizerControlsToggle[testValue]['operator'] : 'AND';
						...
						if ( ! _.isUndefined( conditions ) ) {
							...
							if ( ! control.checkDependency( conditions, values, operator ) ) {
								...
								control_obj.container.removeClass( 'elmispase-hide' );
							}
						}
					}
					...
					$.each(
						...
						function ( index, val ) {
							...
							if ( 'undefined' !== typeof ElmispaseCustomizerControlsToggle[cond_key] ) {
								var conditions = ! _.isUndefined( ElmispaseCustomizerControlsToggle[cond_key]['conditions'] ) ? ElmispaseCustomizerControlsToggle[cond_key]['conditions'] : ElmispaseCustomizerControlsToggle[cond_key];
								var operator   = ! _.isUndefined( ElmispaseCustomizerControlsToggle[cond_key]['operator'] ) ? ElmispaseCustomizerControlsToggle[cond_key]['operator'] : 'AND';
			...
			hideEmptySections : function () {
				...
				$( 'ul.accordion-section.control-section-elmispase_section' ).each(
					function () {
						...
						if ( controls.length > 0 ) {
							controls.each(
								function () {
									if ( ! $( this ).hasClass( 'elmispase-hide' ) && $( this ).css( 'display' ) != 'none' ) {
										visibleIt = true;
									}
								}
							);
							...
							if ( ! visibleIt ) {
								$( '.control-section[aria-owns="' + parentId + '"]' ).addClass( 'elmispase-hide' );
							} else {
								$( '.control-section[aria-owns="' + parentId + '"]' ).removeClass( 'elmispase-hide' );
							}
						}
					}
				);
		...
		$(
			function () {
				Elmispase_Customizer_FrameWork.init();
			}
		);
