<?php
/**
 * Elmispase dynamic CSS generation file for theme options.
 *
 * Class Elmispase_Dynamic_CSS
 *
 * @package    ThemeGrill
...
class Elmispase_Dynamic_CSS {
	...
	public static function render_output( $dynamic_css, $dynamic_css_filtered = '' ) {
		 ...
		 * Typography options.
		 */
		$elmispase_content_font = array(
			'font-family' => 'Lato',
			'font-weight' => 'regular',
		);

		$elmispase_titles_font = array(
			'font-family' => 'Lato',
			'font-weight' => 'regular',
		);


		$base_typography                                     = get_theme_mod( 'elmispase_content_font_typography', $elmispase_content_font );
		$heading_typography                                  = get_theme_mod( 'elmispase_titles_font_typography', $elmispase_titles_font );
		...
		$primary_color_css = array(
			...
			'.elmispase-button, input[type="reset"], input[type="button"], input[type="submit"], button, #featured-slider .slider-read-more-button, #controllers a:hover, #controllers a.active, .pagination span ,.site-header .menu-toggle:hover, ., .comments-area .comment-author-link span, a#back-top:before, .post .entry-meta .read-more-link, a#scroll-up, .search-form span, .main-navigation .tg-header-button-wrap.button-one a' => array(
				'background-color' => esc_html( $primary_color ),
			),
			'.main-small-navigation li:hover, .main-small-navigation ul > .current_page_item, .main-small-navigation ul > .current-menu-item, .elmispase-woocommerce-cart-views .cart-value' => array(
			...
			'.site-header .menu-toggle:hover.entry-meta a.read-more:hover,#featured-slider .slider-read-more-button:hover,.:hover,.entry-meta .read-more-link:hover,.elmispase-button:hover, input[type="reset"]:hover, input[type="button"]:hover, input[type="submit"]:hover, button:hover' => array(
		...
		$parse_css .= elmispase_parse_css( '#0FBE7C', $primary_color, $primary_color_css );

		/**
		 * Typography options.
		 */
		// Base typography.
		$parse_css .= elmispase_parse_typography_css(
		...
		return apply_filters( 'elmispase_theme_dynamic_css', $parse_css );
