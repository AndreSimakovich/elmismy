			...
			function isElementInViewport( subMenu ) {
				...
				if ( 'function' === typeof subMenu.getBoundingClientRect ) {
					...
					if ( rect.right + 2 > ( window.innerWidth || document.documentElement.clientWidth ) ) {
						return 'elmispase-menu--left'; // menu goes out of viewport from right.
					} else if ( rect.left < 0 ) {
						return 'elmispase-menu--right'; // menu goes out of viewport from left.
					} else {
						return false;
					}
				}
	...
	function () {
		...
		jQuery( document ).ready( function () {
			...
			var mainWrapper           = document.querySelector( '#header-text-nav-container .inner-wrap' ),
			    ...
			    headerDisplayTypeFour = document.getElementsByClassName( 'elmispase-header-display-four' )[0];
